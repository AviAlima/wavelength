...
test("full round flow: clue -> guess -> reveal -> next round with alternating giver", async ({ browser }) => {
  ...
  try {
    ...
    const guess = Number(room.round.mapValue.fields.guess.doubleValue);
    const target = Number(room.round.mapValue.fields.target.doubleValue);
...
test("version footer is displayed", async ({ page }: { page: Page }) => {
  ...
  await expect(page.locator("#version")).toHaveText("1.2.0");
});
