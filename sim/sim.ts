...
const eq = (a: unknown, b: unknown): boolean => {
  ...
  return ka.every((k) => Object.hasOwn(b, k) && eq((a as Record<string, unknown>)[k], (b as Record<string, unknown>)[k]));
};
const read = async (db: typeof dbA, code: string): Promise<RoomData> =>
  (await getDoc(doc(db, "rooms", code))).data() as RoomData;
const roomRef = (db: typeof dbA, code: string) => doc(db, "rooms", code);
